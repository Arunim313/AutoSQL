...
class ChromaDB_VectorStore(Logic):
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        ...
        csv_data = self.csv_collection.get()
        if csv_data is not None:
            # Extract the documents and ids
            documents = [doc for doc in csv_data["documents"]]
            ids = csv_data["ids"]
    ...
    def get_related_csv(self, question: str, n_results: int = None, **kwargs) -> list:
        """
        Fetches related documents from the CSV collection based on the provided question.

        Args:
            question (str): The question or query text to search for in the CSV.
            n_results (int, optional): The number of results to return. If None, uses self.n_results.
            **kwargs: Additional keyword arguments for flexibility.

        Returns:
            list: A list of related documents.
        """
        # Use self.n_results if n_results is not provided
        if n_results is None:
            n_results = self.n_results_csv
    
        query_results = self.csv_collection.query(
            query_texts=[question],
            n_results=n_results,  # Use n_results to control how many results to fetch
        )

        # Extract documents using the existing method
        related_documents = ChromaDB_VectorStore._extract_documents(query_results)
